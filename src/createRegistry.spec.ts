...
jest.mock('./fetchXcAssetRegistryInfo', () => ({
	fetchXcAssetsRegistryInfo: jest.fn().mockReturnValueOnce(
		Promise.resolve([
			{
				paraID: 0,
...
jest.mock('./createChainRegistryFromParas', () => ({
	...jest.requireActual('./createChainRegistryFromParas'), // import and retain the original functionalities
...
describe('createRegistry', () => {
	it('Should correctly create the Asset Registry', async () => {
		(fetchParaIds as jest.MockedFunction<any>)
		...
		(createChainRegistryFromRelay as jest.MockedFunction<any>)
			.mockReturnValueOnce(
				Promise.resolve({
					tokens: ['DOT'],
					assetsInfo: {},
		...
		const createdRegistry = JSON.parse(registryData);
